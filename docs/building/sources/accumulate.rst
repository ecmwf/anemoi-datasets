...
- `Option 5: Absolute datetime base-time sequences`_
...
For full control, provide an explicit list of ``(basetime, steps)`` pairs where
``basetime`` is an integer hour of day (0–23) and ``steps`` is a slash-separated
string of ``"start-end"`` step ranges in hours.
...
When using the dict form of Option 4, an optional ``base_date`` key can restrict
which base dates are used.  Currently, ``day_of_month: N`` is supported, which
keeps only intervals whose base falls on day *N* of the month.  This is useful
for monthly-reset accumulations:
...
Option 5: Absolute datetime base-time sequences
------------------------------------------------

Use this option when forecast runs are not anchored to a fixed hour each day but
instead follow an arithmetic sequence of absolute datetimes — for example, a
source whose runs are issued every 18 hours starting from 1970-01-01 00:00.

Provide a list of dicts, each with:

- **start**: the anchor datetime of the sequence (string, e.g. ``"1970-01-01 00:00"``).
- **frequency**: the period between successive base times (e.g. ``"18h"``).
- **steps**: the accumulation steps, using the same ``"start-end"`` syntax as Option 4.

.. literalinclude:: yaml/accumulations-absolute-sequence.yaml
   :language: yaml

The bases generated by this example are
``1970-01-01T00:00``, ``1970-01-01T18:00``, ``1970-01-02T12:00``, … and so on.
Because the 18-hour period does not divide evenly into 24 hours, each calendar
day has a different set of valid base times — something that cannot be expressed
with the ``(hour-of-day, steps)`` pairs of Option 4.

Multiple patterns can be combined by adding more dicts to the list.
