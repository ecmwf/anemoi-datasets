...
You can also read  specific GRIB messages by specifying them using the MARS language (excluding the
keywords `date`, `time` and `step`, as well as any post-processing options, such as `grid` or `area`):
...
.. _wildcards: https://en.wikipedia.org/wiki/Glob\_(programming)
