...
class HTML_Writer:
    ...
    def update_table(self, v1, global_mean, lam_mean, mean_diff, global_std, lam_std, std_diff, global_max, lam_max, max_diff, global_min, lam_min, min_diff):
...
def plot_coordinates_on_map(lats, lons):
    ...
    margin = 10
    ax.set_extent([min(lons) - margin, max(lons) + margin, min(lats)- margin, max(lats) + margin], crs=ccrs.PlateCarree())
...
class CompareLAM(Command):
    ...
    def add_arguments(self, command_parser):
        command_parser.add_argument("dataset1", help="Path of the global dataset. Or path to the largest dataset.")
        command_parser.add_argument("dataset2", help="Path of the LAM dataset. Or path to the smallest dataset.")
        command_parser.add_argument("num_dates", help="Number of datapoints (in time) to compare over.", default=10)
        command_parser.add_argument("outpath", help="Path to output folder", default="./")
        command_parser.add_argument("round_ndigits", help="Number of digits to keep.", default=4)
        command_parser.add_argument("--save_plots", action="store_true", help="Toggle to save a picture of the data grid.")
    ...
    def run(self, args):
        ...
        lam_area = rectangle_area_km2(max(lam_dataset.latitudes), max(lam_dataset.longitudes), min(lam_dataset.latitudes), min(lam_dataset.longitudes))
        l_coords = max(lam_dataset.latitudes), min(lam_dataset.longitudes), min(lam_dataset.latitudes), max(lam_dataset.longitudes)

        if args.save_plots:
            fig = plot_coordinates_on_map(lam_dataset.latitudes, lam_dataset.longitudes, random_gaussian=False)
        ...
        global_area = rectangle_area_km2(max(global_dataset.latitudes), max(global_dataset.longitudes), min(global_dataset.latitudes), min(global_dataset.longitudes))
        ...
        if args.save_plots:
            fig = plot_coordinates_on_map(global_dataset.latitudes, global_dataset.longitudes, random_gaussian=False)
        ...
        table = PrettyTable()
        table.field_names = ["Variable", "Global Mean", "LAM Mean", "Mean Diff (%)", "Global Std", "LAM Std", "Std Diff (%)", "Global Max", "LAM Max", "Max Diff (%)", "Global Min", "LAM Min", "Min Diff (%)", ]
        ...
        for v1, v2 in zip(global_vars, lam_vars): 
            ...
            html_writer.update_table(v1, global_mean, lam_mean, mean_diff, global_std, lam_std, std_diff, global_max, lam_max, max_diff, global_min, lam_min, min_diff)
