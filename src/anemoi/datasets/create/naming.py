...
GRIDDED_PATTERN = (
    r"^(\w+)-([\w-]+)-(\w+)-((?:[0-9on]\w*|r\d\db\d\d))-(\d\d\d\d)-(\d\d\d\d)-" r"(\d+h|\d+m)-v(\d+)-?([a-zA-Z0-9-]+)?$"
)
GRIDDED_KEYS = [
    "purpose",
    "labelling",
...
TRAJECTORIES_PATTERN = (
    r"^(\w+)-([\w-]+)-(\w+)-((?:[0-9on]\w*|r\d\db\d\d))-(\d\d\d\d)-(\d\d\d\d)-"
...
TABULAR_PATTERN = (
    r"^(\w+)-([\w-]+)-(\w+)(?:-((?:[0-9on]\w*|r\d\db\d\d)))?-(\d\d\d\d)-(\d\d\d\d)-" r"v(\d+)-?([a-zA-Z0-9-]+)?$"
)
TABULAR_KEYS = [
    "purpose",
    "labelling",
