...
class InterpolateNearest(Forwards):
    ...
    def __init__(self, dataset, interpolate_variables):
        from ..grids import nearest_grid_points

        super().__init__(dataset)
        self.vars = interpolate_variables
        self.var_mask = {v: np.where(~np.isnan(dataset[0, dataset.name_to_index[v]].squeeze()))[0] for v in interpolate_variables}
        self.dataset = dataset
        self.ngp_var = {v:  nearest_grid_points(
