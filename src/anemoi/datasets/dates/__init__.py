...
class TrajectoryDates(DatesProvider):
    """Dates provider for trajectory datasets.

    ``values`` is the list of ``(basetime, step)`` pairs formed by the Cartesian
    product of a basetimes provider (built via :meth:`DatesProvider.from_config`)
    with a list of forecast steps derived from ``steps['start' | 'end' |
    'frequency']``.  Use :meth:`factorise` to recover the underlying
    sorted-unique basetimes and steps.

    Parameters
    ----------
    steps : dict
        Mapping with ``start``, ``end`` and ``frequency`` keys, as passed via
        ``recipe.steps``.
    **kwargs : Any
        Keyword arguments forwarded to :meth:`DatesProvider.from_config` to
        build the underlying basetimes provider (``start``, ``end``,
        ``frequency``, ``missing``, …).
    """

    def __init__(self, steps: dict, **kwargs: Any) -> None:
        import numpy as np
        ...
        self._steps = np.arange(start, end + frequency, frequency)

        self._basetimes = DatesProvider.from_config(**kwargs)

        self.values = [(bt, st) for bt in self._basetimes.values for st in self._steps]
        # Missing handling is delegated to the basetimes provider; pairs are not
        # masked individually.
        self.missing = list(self._basetimes.missing)
        self.window = None
    ...
    def frequency(self) -> datetime.timedelta:
        ...
        return self._basetimes.frequency
